...
2. Пересобрать нужную редакцию приложения.
3. Запустить собранный файл с `--check-config` (скрипт релиза делает это сам).
...
`--check-config` проверяет ссылки между каталогом, действиями и workflow,
наличие нужных capabilities и артефактов у профилей, доступность стратегий,
файлы прошивок, их SHA-256 и формат HEX. Ненулевой код
завершения означает, что выпускать эту сборку нельзя.
Для `actions.json` и `workflows.json` требуется `schemaVersion: 1`; другая
версия отклоняется при загрузке.

Для новой модели на протоколе `unicorn-ascii` скопируйте запись из `devices`,
задайте уникальные `id` и `type`, параметры регистров и capabilities; затем
добавьте её `firmwareCatalogs` с версиями, файлами и разрешёнными переходами.
Отдельный C++-класс модели не нужен. Для нового действия добавьте запись с
capability и ссылкой на workflow в `actions.json`, а в `workflows.json` — шаги
из `OperationRegistry`. Проверьте режим `--check-config` в обеих редакциях и
сценарий на fake transport. Новый протокол, операция или алгоритм записи
требует узкой C++-реализации и теста; JSON не исполняет произвольный код.

Перед добавлением модели или действия проверьте:

- уникальность ID и соответствие обнаруживаемого `type`;
- необходимые capabilities, регистры и состояние устройства;
- существование workflow и зарегистрированных `op`;
- пути, SHA-256, формат образа и разрешённые переходы;
- шаги проверки артефакта и плана до записи, а также чтение страниц после неё;
- `device.ensureUuid` до первого шага, меняющего состояние устройства;
- `device.verifyRegister` после записи даты или номера и до следующего
  изменяющего шага;
- результат `--check-config` для внутренней и внешней редакций;
- тест fake transport и аппаратную проверку на одном приборе.
...
| `operationParameters` | Номера служебных регистров, включая `applicationLoadRegister` |
...
```json
{
  ...
  "operationParameters": {"applicationLoadRegister": 0},
  "capabilities": [
    "identity.read",
