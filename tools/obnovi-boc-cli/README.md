...
Запустите `releases/tools/obnovi-boc-cli.exe` двойным щелчком. В окне доступны
ровно две кнопки:

- **Перевыпустить собранное приложение** — синхронизирует прошивки и выполняет
  чистую сборку внутренней и внешней редакций;
- **Перевыпустить установочные файлы** — пересобирает обе редакции, затем создаёт
  установщики и portable ZIP версии `1.0.0`.
...
Командная строка сохранена для автоматизации. Чисто пересобрать обе редакции:
